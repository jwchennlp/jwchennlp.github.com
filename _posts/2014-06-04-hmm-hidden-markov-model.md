...
$B_{ij}=p(o_j｜s_i)表示在隐含状态s_i的情况下,观察状态为o_j的概率$.		
...
我们知道计算t时刻隐含状态j的局部概率$\delta_{t}(j)$,需要的知道t-1时刻的$的\delta_{t-1}$,我们需要记录t-1时刻的某个状态i,有i状态到达t时刻的状态j是到达状态j的最佳路径.通过反向指针来记录这种状态:		
